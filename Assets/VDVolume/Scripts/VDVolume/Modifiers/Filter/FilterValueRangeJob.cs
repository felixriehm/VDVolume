...
{
    /// <summary>
    /// For a color value range, this job will set 'visible' and 'solid' voxels in the dense storage to 'removed'.
    /// </summary>
    internal struct FilterValueRangeJob : IJob
    {
        ...
        private int _rangeBeginIncl;
        private int _rangeEndIncl;

        internal FilterValueRangeJob(Vector3Int start, Vector3Int end, GridDataRaw gridData, DenseDataRaw denseData, int rangeBeginIncl, int rangeEndIncl, int chunkIndex, NativeArray<bool> modifiedChunks,NativeArray<bool> modifiedChunksWithSurfaceRemoval)
        {
            _start = start;
            _end = end;
            _gridData = gridData;
            _denseData = denseData;
            _rangeBeginIncl = rangeBeginIncl;
            _rangeEndIncl = rangeEndIncl;
            ...
            for (int x = _start.x; x < _end.x; x++)
            {
                ...
                for (int y = _start.y; y < _end.y; y++)
                {
                    ...
                    for (int z = _start.z; z < _end.z; z++)
                    {
                        ...
                        if (voxelData.Color >= _rangeBeginIncl && voxelData.Color <= _rangeEndIncl)
                        {
